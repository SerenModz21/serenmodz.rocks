...
function AnimatedBars() {
  ...
  // return (
  //   <div className="flex w-auto items-end overflow-hidden">
  //     <span id="bar1" className="h-2 w-1 opacity-75" />
  //     <span id="bar2" className="mr-[3px] h-1 w-1 bg-gray-300" />
  //     <span id="bar3" className="h-3 w-1 bg-gray-300 opacity-80" />
  //   </div>
  // );
}
...
export function NowPlaying() {
  ...
  return (
    <div className="mb-2 flex flex-row items-center space-x-5 p-5 sm:space-x-2 sm:p-0">
      {data?.songUrl ? <AnimatedBars /> : <SpotifyIcon />}
      <div className="inline-flex w-full max-w-full flex-col truncate sm:flex-row">
        {data?.songUrl ? (
          <a
            className="capsize max-w-max max-w-max truncate font-medium"
            href={data.songUrl}
            target="_blank"
